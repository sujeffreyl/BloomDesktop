	...
	public class HtmlErrorReporter: IErrorReporter
	{
		...
		public ErrorResult NotifyUserOfProblem(IRepeatNoticePolicy policy, string alternateButton1Label, ErrorResult resultIfAlternateButtonPressed, string message)
		{
			...
			finally
			{
				...
				// in order for other threads to be able to acquire the ock later.
				System.Threading.Monitor.Exit(_lock);
			}
		}

		// ENHANCE: I think it would be good if ProblemReportApi could be split out.
		// Part of it is related to serving the API requests need to make the Problem Report Dialog work. That should stay in ProblemReportApi.cs.
		...
		{
			// Note: I think it's better to call ProblemReportApi directly insetad of through NonFatalProblem first.
		...
		{
			// REVIEW: Is GetControlToUse() logic OK for getting which form to use?
				...
				try
				{
					...
					using (var dlg = BrowserDialogFactory.CreateBrowserDialog(url))
					{
						...
						try
						{
							...
							// This code has been implemented to fllow that model
							//
